...
CONTROL_MODE = os.getenv("HYTALE_CONTROL_MODE", "k8s").lower()
SYSTEMD_SERVICE = os.getenv("HYTALE_SERVER_SERVICE", "hytale-server")
...
def _fetch_systemd_status():
    ...
    if result.returncode == 0:
        return True, detail or "active"
    return False, detail or "inactive"


def _fetch_systemd_logs(lines=10):
    try:
        result = subprocess.run(
            ["/usr/bin/docker", "logs", "--tail", str(lines), SYSTEMD_SERVICE],
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
...
def _fetch_pod_status():
    if CONTROL_MODE == "systemd":
        return _fetch_systemd_status()
...
def _fetch_last_logs(lines=10):
    if CONTROL_MODE == "systemd":
        return _fetch_systemd_logs(lines)
...
def _scale_statefulset(replicas):
    if CONTROL_MODE == "systemd":
        action = "start" if replicas > 0 else "stop"
        return _systemd_action(action)
